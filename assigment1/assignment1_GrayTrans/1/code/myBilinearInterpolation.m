...
%call: myBilinearInterpolation(a)
function outImg = myBilinearInterpolation( img )
...
function intensity=interpolateIntensity(img,x,y)
    ...
    %applying bilinear interpolation, in MATRIX form. 
    % Reference: https://en.wikipedia.org/wiki/Bilinear_interpolation#Alternative_algorithm    
    mat1=[1,x1,y1,x1*y1;1,x1,y2,x1*y2;1,x2,y1,x2*y1;1,x2,y2,x2*y2];
    mat1=inv(mat1)';%inverse and transpose
    colVec=[1,x,y,x*y]';% column vector
    
    %b=[b11,b12,b21,b21]^T : Column Vector
    b=mat1*colVec;
    
    %Here x1,x2,y1,y2 are w.r.t Output image whose dimension is 3m-2 x 2n-1
    %There converting back to Input image dimension.
    iX1=(x1+mOffset)/mTime;iY1=(y1+nOffset)/nTime;
    iX2=(x2+mOffset)/mTime;iY2=(y2+nOffset)/nTime;
    %fprintf('iX1=%d iY1=%d\niX2=%d iY2=%d\n',iX1,iY1,iX2,iY2);
    
    %fval=[f(Q11),f(Q12),f(Q21),f(Q22)]: Row Vector    
    fval=[img(iX1,iY1),img(iX1,iY2),img(iX2,iY1),img(iX2,iY2)];
    
    %f(x,y) = b_{11}f(Q_{11})+b_{12}f(Q_{12})+b_{21}f(Q_{21})+b_{22}f(Q_{22})    
    intensity=double(fval)*b;       
end
